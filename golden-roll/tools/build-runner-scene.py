...
def build_actor():
    ...
    shadow=image(b,push,'Shadow','particle',4,-58,135,210,(57,31,11,160))
    ball=image(b,push,'Ball','coin',0,0,112,112)
    ballsp=findcomp(b,ball,'cc.Sprite'); b.nodes[ballsp]['_customMaterial']={'__uuid__':uid('hit-material'),'__expectedType__':'cc.Material'}
    beetle=b.node('BackViewSpine',push,130,156,0,-108)
    sk=component(b,beetle,'sp.Skeleton',_skeletonData=None,_defaultSkinIndex=0,_animationIndex=0,_skinName='default',_animationName='',_cacheMode=0,_premultipliedAlpha=True,_timeScale=1,_loop=True,_enableBatch=False,_color=H.color())
    component(b,n,script('RunnerActorView'),skeleton=ref(sk),ball=ref(ballsp),opacity=ref(op),dust=ref(dust),aura=ref(aura),pushRoot=ref(push))
...
def build_scene(item,actor,burst,modal):
    ...
    hudview=component(b,hud,script('RunnerHudView'),coins=ref(coins),distance=ref(distance),best=ref(best),mission=ref(missionlabel),hint=ref(hint),hintOpacity=ref(hintop),progress=ref(fsp),hearts=hearts,powerCounts=counts,powerButtons=powerbuttons,left=ref(left),right=ref(right),pause=ref(pause))
